...
const Products = () => {
  ...
  // 🧪 Optional: Make the fade effect dynamic using scroll
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Scroll progress through the fade-section (0 = top of screen, 1 = fully in view)
      const progress = Math.min(1, Math.max(0, (windowHeight - rect.top) / windowHeight));
      const opacity = 1 - progress; // yellow fades away as section scrolls in
